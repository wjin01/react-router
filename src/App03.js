...
function App() {
  ...
  const handleClick = async () => {
    ...
    //async(비동기야), await(기다려)
    //1. async함수 안에서만 await키워드를 쓸 수 있음
    //2. 민약 function 앞에 async를 붙이면 언제나 Promise객체를 반환함
    //3. 함수의 리턴이 Promise라면 await키워드를 붙이거 then을 생략할 수 있음
  ...
  //실습 - 힌트는 즉시실행 함수를 사용함
  //화면 로드시에 hi.json 데이터를 async, await로 가져오는 프로그램 코드 작성
